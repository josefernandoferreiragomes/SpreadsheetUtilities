...
## Phase 8 — Session Storage Location Selector ?

**Scope:** Application + Infrastructure + Presentation (UI.Web)
**Goal:** Make session storage configurable at runtime with a UI selector on the Session Admin page
**Status:** ? Complete (2026-06-13)

| # | Step | Result |
|---|---|---|
| **8.1** | Create `ISessionStorage` port interface in `Application/Ports/` with 4 methods matching `IAuthService` | ? Created |
| **8.2** | Create `SessionStorageLocation` enum in `Application/Configuration/` with 3 values: `UiWebMemoryCache`, `AuthApiMemoryCache`, `RedisCache` | ? Created |
| **8.3** | Implement `AuthApiSessionStorage` in `Infrastructure/Services/` — delegates to Auth.Api via `SpreadsheetUtilitiesAuthApiClient` | ? Created |
| **8.4** | Implement `LocalMemorySessionStorage` in `Infrastructure/Services/` — local `IMemoryCache` (same pattern as `AuthService`) | ? Created |
| **8.5** | Implement `RedisSessionStorage` stub in `Infrastructure/Services/` — all methods throw `NotImplementedException` | ? Created |
| **8.6** | Implement `SessionStorageSelector` — runtime resolver that caches current location in `IMemoryCache` and resolves the correct `ISessionStorage` from DI | ? Created |
| **8.7** | Refactor `SessionService` to use `SessionStorageSelector` instead of creating `HttpClient`/`SpreadsheetUtilitiesAuthApiClient` directly | ? Done — removes direct NSwag dependency from SessionService |
| **8.8** | Update `SessionAdmin.razor` with storage location selector card (dropdown + apply button + status messages) | ? Done |
| **8.9** | Register new services (`AuthApiSessionStorage`, `LocalMemorySessionStorage`, `RedisSessionStorage`, `SessionStorageSelector`) in `UI.Web/Program.cs` | ? Done |
...
## Phase 7 - Code Analysis Warning Cleanup

**Scope:** All projects  
**Goal:** Eliminate all code analysis warnings (CA1848, CS8625, WHITESPACE) and suppress NSwag auto-generated warnings at project level  
**Status:** Complete (2026-06-10)

| # | Step | Result |
|---|---|---|
| **7.1** | Fix WHITESPACE formatting in DoubleEntrySpreadsheetGeneratorSimplified.cs via dotnet format | Done |
| **7.2** | Fix CS8625: Change IAuthService.GetSession() return type from string to string? to properly model nullable session | Done |
| **7.3** | Move NSwag #pragma warning disable from auto-generated code to project-level <NoWarn> in Infrastructure .csproj | Done |
| **7.4** | Fix CA1848 in LoggingBehavior.cs: Replace logger.LogDebug() calls with [LoggerMessage] source-generated delegates (2 instances) | Done |
| **7.5** | Fix CA1848 in FolderExampleFileProvider.cs: Replace logger.Log*() calls with [LoggerMessage] source-generated delegates (7 instances) | Done |
| **7.6** | Fix CA1848 in ExampleFilesEndpoints.cs: Replace logger.Log*() calls with [LoggerMessage] source-generated delegates (11 instances) | Done |
| **7.7** | dotnet build / dotnet format --verify-no-changes - 0 warnings, 0 style errors | Done |
| **7.8** | dotnet test - 71/71 all tests pass | Done |
| | **Final metrics** | Build: 0 errors, 0 warnings, dotnet format clean. Tests: 71 pass, 0 failures |

### Design Decisions

| Decision | Choice |
|---|---|
| Logging pattern | [LoggerMessage] source generator attributes (not manual LoggerMessage.Define) |
| Nullable modeling | Interface returns string? when value can be absent |
| Auto-generated code | Project-level <NoWarn> instead of inline pragmas |
